 ...
 */
function findAdjacentCommentRange(
  document: vscode.TextDocument,
  rangeEnd: vscode.Position
): { range: vscode.Range; whitespaceRange: vscode.Range } | null {
...
async function convertGrayScaleHandler() {
  ...
  if (grayColorsToConvert.length === 0) {
    vscode.window.showInformationMessage(
      "No recognized Tailwind gray scale colors found to convert."
    );
    return;
  }
  ...
  const targetScale = await vscode.window.showQuickPick(
    GRAY_SCALE_NAMES.map(name => ({
      label: `Convert all grays to ${name}`,
      scale: name,
    })),
  ...
  for (let i = grayColorsToConvert.length - 1; i >= 0; i--) {
    ...
    const targetColorEntry = colorMap.find(
      entry => entry.name === targetColorName
    );

    if (targetColorEntry) {
      // 4. Construct replacement strings using helpers
      const replacementOklchString = createOklchString(
        targetColorEntry,
        grayInfo.alphaString
      );
      const replacementCommentString = createAnnotationComment(
        targetColorName,
        grayInfo.alpha
      );
      ...
      console.warn(
        `Could not find target color: ${targetColorName} for original ${grayInfo.originalName}`
      );
    }
  }
  ...
  if (convertedCount > 0) {
    ...
    if (success) {
      vscode.window.showInformationMessage(
        `Converted ${convertedCount} gray scale color(s) to ${targetScaleName}.`
      );
    } else {
    ...
    vscode.window.showInformationMessage(
      "No colors were converted (target shades might be missing)."
    );
  }
}
...
async function removeOklchColorAnnotationsHandler() {
  ...
  if (commentsRemovedCount > 0) {
    ...
    if (success) {
      vscode.window.showInformationMessage(
        `Removed ${commentsRemovedCount} OKLCH color annotation(s).`
      );
    } else {
    ...
    vscode.window.showInformationMessage(
      "No OKLCH Annotations found to remove."
    );
  }
}
...
async function annotateOklchColorsHandler() {
  ...
  for (let i = parsedColors.length - 1; i >= 0; i--) {
    ...
    const commentString = createAnnotationComment(
  ...
  if (success) {
    vscode.window.showInformationMessage(
      `Annotated ${parsedColors.length} OKLCH color(s).`
    );
  } else {
...
function parseOklchFunctionsInRange(
  document: vscode.TextDocument,
  range: vscode.Range
): ParsedOklch[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (!isNaN(l) && !isNaN(c) && !isNaN(h)) {
      ...
      const absoluteStartOffset =
        document.offsetAt(range.start) + matchStartOffset;
 ...
 */
async function selectColorHandler(
  documentUri: vscode.Uri,
  targetRange: vscode.Range,
  originalAlphaString: string | undefined
) {
...
export class OklchColorActionProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    const parsedColorsOnLine = parseOklchFunctionsInRange(
      document,
      currentLine.range
    );
    ...
    for (const parsedColor of parsedColorsOnLine) {
      if (range instanceof vscode.Selection) {
        if (parsedColor.range.intersection(range)) {
    ...
    action.command = {
      ...
      arguments: [
        document.uri,
        {
          // Pass range data
          start: {
            line: targetColor.range.start.line,
            character: targetColor.range.start.character,
          },
          end: {
            line: targetColor.range.end.line,
            character: targetColor.range.end.character,
          },
        },
...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "shadcn-color-tool" is now active!'
  );
    ...
    vscode.commands.registerCommand(
      ...
      (
        ...
        const targetRange = new vscode.Range(
          new vscode.Position(
            targetRangeData.start.line,
            targetRangeData.start.character
          ),
          new vscode.Position(
            targetRangeData.end.line,
            targetRangeData.end.character
          )
        );
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      ["css", "tailwindcss"],
      new OklchColorActionProvider(),
      {
        providedCodeActionKinds:
          OklchColorActionProvider.providedCodeActionKinds,
      }
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "shadcn-color-tool.annotateColors",
      annotateOklchColorsHandler
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "shadcn-color-tool.removeColorAnnotations",
      removeOklchColorAnnotationsHandler
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "shadcn-color-tool.convertGrayScale",
      convertGrayScaleHandler
    )
  );
}
